...
public class Nexus : MonoBehaviour
{
    ...
    private void Awake()
    {  //Empieza con el indice en 0, establece la camara principal y busca los componentes que necesita
        cam = Camera.main;
    ...
    private void Update()
    {
        if (!pauseState)
        {
            if (isDraggin)
            {
                ...
                {        //Muevo el misil y el cursor a donde esta el mouse y agrando el collider del nexo
                ...
                {      //Marco el punto donde se solto para calcular el disparo
                    if (UseEnergy(missiles[index].energyConsumption))
                    {   //Pregunto si tengo esa energia 
                    ...
                    {   //Si no tengo la energia para lanzarlo reseteo su posicion
                        NexusRestore();
    ...
    {
        if (haveMissile && powerIndex == 0 && towerIndex == 0 && !pauseState)
        {  //Si tiene un misil activo y ningun poder o torre activo
            ShowFeedback();
            if (Input.GetMouseButtonDown(0))
            {        //Marco el punto de origen
    ...
    {   //Si tengo la energia suficiente para lanzarlo, le resto la energia
    ...
    {   //Esta funcion recibe el valor de la energia del nexusStats y lo guarda, y al cambiar 
        //el valor de la energia, le manda ese valor al nexusStats para que lo actualize
        currentEnergy.Value = amount;
    }
    public void SetStabilityValue(int amount)
    {  //Guarda la estabilidad base del NexusStats
        baseStability = amount;
    }
    public void SetSpeedValue(int amount)
    {  //guardo la velocidad Base del nexusStats
        baseSpeed = amount;
    }
    ...
    {///Muevo los objetos en donde esta el mouse
    ...
    {//Desactiva a todos los suscriptores y los objetos
        DisableNexus();
    }
