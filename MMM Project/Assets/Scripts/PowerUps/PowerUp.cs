...
public class PowerUp : MonoBehaviour
{
    ...
    {   //Se establece el tiempo que va a estar activo
        timer = cooldown;
    }
    ...
    {
        if (timer < 0)
        {  //Se reduce el tiempo hasta que sea menor a 0 y ahi se desactiva, y se actualiza la imagen
    ...
    {    //Se activa la imagen de cuando esta el mouse encima y si se presiona el mouse llama a las actions que tiene
    ...
    {    //Se desactiva la imagen de cuando esta el mouse encima
        cooldownFeedback.gameObject.SetActive(false);
    }

    #region Actions 
    public void EnergyPowerUp(int cooldown)
    {    //Llama a la funcion de EnergyPowerUp del nexusStats
        //Debug.Log("Energy");
    ...
    { //Llama a la funcion de StructurePowerUp del nexusStats
        NexusStats nexus = FindObjectOfType<NexusStats>();
        nexus.StructurePowerUp(cooldown);
    }

    public void StabilityPowerUp(int cooldown)
    { //Llama a la funcion de StabilityPowerUp del nexusStats
        NexusStats nexus = FindObjectOfType<NexusStats>();
        nexus.StabilityPowerUp(cooldown);
    }
    public void SpeedPowerUp(int cooldown)
    { //Llama a la funcion de SpeedPowerUp del nexusStats
        NexusStats nexus = FindObjectOfType<NexusStats>();
        nexus.SpeedPowerUp(cooldown);
    }
    public void CooldownPowerUp(int cooldown)
    {  //Llama a la funcion de CooldownPowerUp del nexusStats
        NexusStats nexus = FindObjectOfType<NexusStats>();
        nexus.CooldownPowerUp(cooldown);
    }
    public void NuclearPowerUp()
    {   //Deshabilita nuevos inputs del inputController y cambia el indice de poderes al del poder Nuclear
        InputController inputController = FindObjectOfType<InputController>();
        inputController.SetPowerIndex(6);
        inputController.isAvailable = false;
    }
    public void SingularityPowerUp()
    {   //Deshabilita nuevos inputs del inputController y cambia el indice de poderes al del poder Singularity
        InputController inputController = FindObjectOfType<InputController>();
        inputController.SetPowerIndex(7);
        inputController.isAvailable = false;
    }
    public void AntimatterPowerUp(int cooldown)
    {    //Deshabilita nuevos inputs del inputController para cambiar misiles y establece los misiles antimateria
        InputController inputController = FindObjectOfType<InputController>();
