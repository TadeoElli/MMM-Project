...
{
    [SerializeField] private List<GameObject> towerPrefab;        //Lista de misiles
    ...
    private void Awake() {
        ...
        foreach (GameObject prefabs in towerPrefab)       //Creo el diccionario poniendole a cada prefab en la lista una lista de la cantidad de misiles generados como valor a devolver
        {
            towerDictionary.Add(prefabs, new List<GameObject>());
        }
    }
    ...
    public void AddTowersToPool(int amount, GameObject prefab){       //Le mando cuantos genero y cual misil

        List<GameObject> prefabList = towerDictionary[prefab];    //Guardo la lista de cantidad de misiles en otra lista
    ...
    public GameObject RequestTower(GameObject prefab){        //Le mando cual necesito

        List<GameObject> prefabList = towerDictionary[prefab];
