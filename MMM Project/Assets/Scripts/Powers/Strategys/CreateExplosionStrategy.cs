...
public class CreateExplosionStrategy : PowerStrategy
{
    ...
    {    //El comportamiento al presionar, si devuelve true pasa al siguiente estado
        return true;
    }
    ...
    {  //El comportamiento al mantener presionado, si devuelve true es porque no tiene comportamiento mientras se presiona
        return true;
    }
    public override void BehaviourEnded()
    {  //El comportamiento al soltar
        Vector2 origin = Camera.main.ScreenToWorldPoint(Input.mousePosition);
        CreateExplosion(origin);
    }

    private void CreateExplosion(Vector2 origin)
    {   //Crea la explosion correspondiente
