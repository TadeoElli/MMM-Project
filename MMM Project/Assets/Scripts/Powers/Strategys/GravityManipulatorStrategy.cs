...
public class GravityManipulatorStrategy : PowerStrategy
{
    ...
    {    //El comportamiento cuando se presiona
        ...
        if (hit.collider != null && hit.collider.CompareTag("Enemy"))
        {
            ...
            if (CheckEnemyLevel(hit.collider.gameObject.layer, level))
            {
    ...
    private bool CheckEnemyLevel(int enemyLayer, int level)
    {
    ...
    {//Toma el componente EnemiBehaviour del enemigo y le establece que no se puede mover, ademas de que desactiva su rigidBody
    ...
    {  //Mientras se mantiene presionado y el enemigo que guardo existe
        if (enemy != null)
        {
    ...
    {  //cuando se deja de presionar, se activa la posibilida de moderse del enemigo
        if (enemy != null)
        {
            enemy.canMove = true;
